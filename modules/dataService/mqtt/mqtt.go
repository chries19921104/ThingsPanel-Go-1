...
func Listen(broker, username, password, clientid string, msgProc func(c mqtt.Client, m mqtt.Message), msgProcOther func(c mqtt.Client, m mqtt.Message), gatewayMsgProc func(c mqtt.Client, m mqtt.Message), otaProgressMsgProc func(c mqtt.Client, m mqtt.Message), otaToinfromMsgProc func(c mqtt.Client, m mqtt.Message),eventMsgProc func(c mqtt.Client, m mqtt.Message)) (err error) {
	running = false
	if _client == nil {
		// 掉线重连
		var connectLostHandler mqtt.ConnectionLostHandler = func(client mqtt.Client, err error) {
			...
			for {
				...
				if !_client.IsConnectionOpen() {
					...
					subscribe(msgProcOther, gatewayMsgProc, otaProgressMsgProc, otaToinfromMsgProc, eventMsgProc)
					break
				}
			}
		}
...
// mqtt订阅
func subscribe(msgProcOther func(c mqtt.Client, m mqtt.Message), gatewayMsgProc func(c mqtt.Client, m mqtt.Message), otaProgressMsgProc func(c mqtt.Client, m mqtt.Message), otaToinfromMsgProc func(c mqtt.Client, m mqtt.Message), eventMsgProc func(c mqtt.Client, m mqtt.Message)) {
	// 订阅默认，直连设备
